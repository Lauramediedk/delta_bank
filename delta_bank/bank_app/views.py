...
from .forms import TransferForm, UserForm, CustomerForm, NewUserForm, NewAccountForm, MessageForm
from .models import Account, Ledger, Customer, Message
...
def message(request):
    ...
    if request.method == 'POST':
        ...
        if message_form.is_valid():
            sender = request.user
            receiver = User.objects.get(pk=message_form.cleaned_data['receiver'].pk)
    ...
    # users = User.objects.exclude(id=request.user.id).filter(is_staff=request.user.is_staff)
    # return render(request, 'bank_app/inbox.html', {'form': form, 'users': users})
